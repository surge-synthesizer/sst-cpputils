...
namespace
{
template <typename Fun> struct remove_noexcept
{
    using type = Fun;
};

template <typename Ret, typename... Args> struct remove_noexcept<Ret(Args...) noexcept>
{
    using type = Ret(Args...);
};

template <typename Fun> using remove_noexcept_t = typename remove_noexcept<Fun>::type;

template <typename Fun>
std::function<remove_noexcept_t<Fun>>
make_std_function(Fun &f) // ref prevents decay to pointer (no move needed)
{
    return std::function<remove_noexcept_t<typename std::remove_reference<Fun>::type>>{f}; /*
...
{
    {
        ...
        const auto guard = make_scope_guard([]() noexcept { incc(lambda_no_capture_count); });
          ...
          "its callback at all.")
{
    {
        resetc(lambda_no_capture_count);
        auto guard = make_scope_guard([]() noexcept { incc(lambda_no_capture_count); });
        REQUIRE_FALSE(lambda_no_capture_count);
...
TEST_CASE("A lambda function with capture can be used to create a scope_guard.")
{
    auto f = 0.0f;
    const auto i = -1;
    std::ignore = make_scope_guard([&f, i]() noexcept { f = static_cast<float>(*&i); });
}
          ...
          "leaving scope.")
{
    ...
    {
        const auto guard = make_scope_guard([&lambda_count]() noexcept { incc(lambda_count); });
          ...
          "callback at all.")
{
    ...
    {
        auto guard = make_scope_guard([&lambda_count]() noexcept { incc(lambda_count); });
        REQUIRE_FALSE(lambda_count);
          ...
          "once when leaving scope.")
{
    ...
    {
        const auto guard = make_scope_guard(std::bind(incc, std::ref(boundf_count)));
          ...
          "once when leaving scope.")
{
...

    {
        ...
        const auto guard = make_scope_guard(std::bind(incc_l, std::ref(boundl_count)));
...
struct virtual_method_holder_intermediate : public virtual_method_holder_pure_base
{
    void virtual_inc_method() noexcept override { m_count += 2; }
};
          ...
          "method exactly once when leaving scope")
{
    ...
    {
        const auto guard = make_scope_guard([&h]() noexcept { h.regular_inc_method(); });
...
{
    regular_method_holder h{};
    std::ignore = make_scope_guard(std::bind(&regular_method_holder::regular_inc_method, h));
}
          ...
          "method exactly once when leaving scope")
{
    ...
    {
        const auto guard = make_scope_guard([&h]() noexcept { h.const_inc_method(); });
        REQUIRE_FALSE(h.m_count);
    }
...
{
    const const_method_holder h{};
    std::ignore = make_scope_guard(std::bind(&const_method_holder::const_inc_method, h));
}
          ...
          "exactly once when leaving scope")
{
    ...
    {
        const auto guard = make_scope_guard(std::bind(&const_method_holder::const_inc_method, &h));
        REQUIRE_FALSE(h.m_count);
    }
          ...
          "its callback at all.")
{
    ...
    {
        auto guard = make_scope_guard(std::bind(&const_method_holder::const_inc_method, &h));
        REQUIRE_FALSE(h.m_count);
          ...
          "virtual method exactly once when leaving scope")
{
    ...
    {
        auto guard = make_scope_guard([&h_base]() noexcept { h_base.virtual_inc_method(); });
...
void fallback() noexcept { goodbye_said = false; }
...
template <typename Stream> struct sclosr
{
    Stream &m_s;

    auto operator()() noexcept(noexcept(m_s.close())) -> decltype(std::declval<Stream>().close())
    {
        return m_s.close();
    }
};
...
auto get_closing_guard(Stream &s) -> decltype(make_scope_guard(sclosr<Stream>{s})) /* not using
                                     lambda because it can't appear in unevaluated contexts; not
                                     using bind because it does not preserve noexcept */
{
...
template <typename T>
void sfinae_tester_impl(T && /*ignored*/, ... /* less specific, so 2nd choice */)
{
    std::ignore = make_scope_guard(inc);
}

template <typename T> void sfinae_tester(T &&t)
{
          ...
          "from without a compilation error")
{
    ...
    sfinae_tester([](float, bool, tag_prefered_overload, virtual_method_holder) noexcept {});
    REQUIRE(count == 2u);
}
...
TEST_CASE("Test nested scopes")
{
    ...
    const auto lvl0_guard = make_scope_guard([&lvl0_count]() noexcept { incc(lvl0_count); });
    REQUIRE_FALSE(lvl0_count);

    {
        const auto lvl1_guard = make_scope_guard([&lvl1_count]() noexcept { incc(lvl1_count); });
...
namespace
{
template <typename Callback> struct scope_guard_holder
{
...
////////////////////////////////////////////////////////////////////////////////
TEST_CASE("Early Invocation")
{
    bool x = false;
    {
        auto guard = make_scope_guard([&]() { x = true; });
        guard.invoke();
        REQUIRE(x);
        x = false;
    }
