...
#ifndef INCLUDE_SST_CPPUTILS_ITERATORS_H
#define INCLUDE_SST_CPPUTILS_ITERATORS_H
 ...
 */
template <typename T, typename S, typename TIter = decltype(std::begin(std::declval<T>())),
          typename = decltype(std::end(std::declval<T>())),
          typename SIter = decltype(std::begin(std::declval<S>())),
          typename = decltype(std::end(std::declval<S>()))>
constexpr auto zip(T &&iterableT, S &&iterableS)
{
    ...
    struct iterable_wrapper
    {
        ...
        auto begin()
        {
            return iterator{std::begin(titerable), std::begin(siterable), std::end(titerable),
                            std::end(siterable)};
        }
        auto end()
        {
            return iterator{std::end(titerable), std::end(siterable), std::end(titerable),
                            std::end(siterable)};
        }
    };
