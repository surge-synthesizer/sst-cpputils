 ...
 *   const auto guard = sst::cpputils::make_scope_guard(
 *       [&]() { fclose(f); });
 *
 * Now when the variable `guard' goes out of scope, it will execute the lambda.
 *
 * Cancel it later:
 *    guard.dismiss();
 *
 * Call it early (will not run when it goes out of scope):
 *    guard.invoke();
 */
...
template <typename A, typename B> struct and_t<A, B> : public std::conditional<A::value, B, A>::type
{
}; // for two arguments
...
struct is_proper_sg_callback_t
    : public and_t<is_noarg_callable_t<T>, returns_void_t<T>, is_nothrow_invocable_if_required_t<T>,
...
template <typename Callback> class [[nodiscard]] scope_guard<Callback> final
{
  public:
...
////////////////////////////////////////////////////////////////////////////////
template <typename Callback>
detail::scope_guard<Callback>::scope_guard::~scope_guard() noexcept /*
...
////////////////////////////////////////////////////////////////////////////////
template <typename Callback> inline void detail::scope_guard<Callback>::dismiss() noexcept
{
    m_active = false;
}

////////////////////////////////////////////////////////////////////////////////
template <typename Callback> inline void detail::scope_guard<Callback>::invoke() noexcept
{
    if (m_active)
        m_callback();
    m_active = false;
